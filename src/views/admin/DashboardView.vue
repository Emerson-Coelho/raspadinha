...
<template>
  <div class="dashboard-container">
    ...
    <div>
      ...
      <div v-if="isLoading" class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4 mb-8">
      ...
      <div v-else class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4 mb-8">
        ...
        <!-- Total de Usuários -->
        <div class="bg-gray-800 rounded-lg p-6 shadow-lg">
          <h3 class="text-gray-400 text-sm font-medium mb-2">Total de Usuários</h3>
          <div class="flex items-center">
            ...
            <table class="w-full">
              <thead>
                <tr class="bg-gray-700">
                  <th class="px-4 py-3 text-left text-xs font-medium text-gray-300 uppercase tracking-wider">Usuário</th>
                  <th class="px-4 py-3 text-left text-xs font-medium text-gray-300 uppercase tracking-wider">Tipo</th>
                  <th class="px-4 py-3 text-left text-xs font-medium text-gray-300 uppercase tracking-wider">Valor</th>
              ...
              <tbody class="divide-y divide-gray-700">
                <tr v-for="transaction in recentTransactions" :key="transaction.id" class="hover:bg-gray-700">
                  <td class="px-4 py-3 whitespace-nowrap text-sm text-white">{{ transaction.user }}</td>
                  ...
                  <td class="px-4 py-3 whitespace-nowrap text-sm text-white">{{ formatCurrency(transaction.amount) }}</td>
                  <td class="px-4 py-3 whitespace-nowrap text-sm text-gray-300">{{ transaction.date }}</td>
                </tr>
              </tbody>
            </table>
          </div>
        ...
        <div class="bg-gray-800 rounded-lg shadow-lg overflow-hidden">
          ...
          <div class="overflow-x-auto">
            <table class="w-full">
              <thead>
                <tr class="bg-gray-700">
                  <th class="px-4 py-3 text-left text-xs font-medium text-gray-300 uppercase tracking-wider">Jogo</th>
                  <th class="px-4 py-3 text-left text-xs font-medium text-gray-300 uppercase tracking-wider">Jogadas</th>
                  <th class="px-4 py-3 text-left text-xs font-medium text-gray-300 uppercase tracking-wider">Receita</th>
                </tr>
              </thead>
              <tbody class="divide-y divide-gray-700">
                <tr v-for="game in topGames" :key="game.id" class="hover:bg-gray-700">
                  <td class="px-4 py-3 whitespace-nowrap text-sm text-white">{{ game.name }}</td>
                  <td class="px-4 py-3 whitespace-nowrap text-sm text-white">{{ game.plays.toLocaleString() }}</td>
                  <td class="px-4 py-3 whitespace-nowrap text-sm text-white">{{ formatCurrency(game.revenue) }}</td>
                </tr>
              </tbody>
            </table>
          </div>
