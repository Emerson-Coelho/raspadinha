...
echo -e "${YELLOW}Iniciando script para criar a tabela de logs do sistema...${NC}"
...
# Executar o script
echo -e "${YELLOW}Executando script de criação da tabela...${NC}"
node createSystemLogsTable.js

# Verificar se o script foi executado com sucesso
if [ $? -eq 0 ]; then
    echo -e "${GREEN}Script executado com sucesso!${NC}"
    echo -e "${GREEN}A tabela de logs do sistema foi criada.${NC}"
    exit 0
else
